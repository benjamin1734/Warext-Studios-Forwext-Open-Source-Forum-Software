...
final class ModerationApplicationFactory
{
    ...
    public function handle(Request $request): ?Response
    {
        ...
        $taskRepository = new DatabaseModerationTaskRepository($this->database());
        $handler = new ModerationWorkspaceHandler(
            ...
            new ModerationTaskService(
                ...
                $taskRepository,
                $gate,
                new DatabaseModerationAuditStore($this->database()),
            ),
            new ModerationRequestGuard($this->canonicalUrl),
            $this->basePath,
            $gate->allows(PermissionKey::fromString('moderation.manage')),
        );
        ...
        } catch (ModerationTaskNotFoundException) {
            return $this->secure(Response::text('Not Found', 404));
