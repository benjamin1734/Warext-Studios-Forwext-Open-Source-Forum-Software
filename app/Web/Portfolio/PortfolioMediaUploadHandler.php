...
final readonly class PortfolioMediaUploadHandler implements RequestHandlerInterface
{
    ...
    public function handle(Request $request): Response
    {
        ...
        $projectId = $this->routeId($request, 'projectId');
        if ($projectId === null) {
        ...
        try {
            ...
            if (($body['action'] ?? null) === 'delete') {
                ...
                if (!is_string($mediaId) || preg_match('/^[a-f0-9]{32}$/D', $mediaId) !== 1) {
                    throw new InvalidArgumentException('Portfolio media id is invalid.');
                }
                $this->media->delete($actor, $projectId, EntityId::fromString($mediaId));
            } else {
                $upload = $request->uploads()['file'] ?? null;
                if (!$upload instanceof UploadedFile) {
                    throw new InvalidArgumentException('Portfolio media upload is missing.');
                }
                $alt = $body['alt'] ?? '';
                if (!is_string($alt)) {
                ...
                $this->media->upload(
                    $actor,
                    $projectId,
                    $this->uploads->read($upload, $this->quota->maxFileBytes),
                    trim($alt),
            ...
            return Response::redirect(
                $this->basePath->prepend('/portfolio/manage?updated=1&project=' . rawurlencode($projectId->value())),
                303,
            ...
            return Response::text('Media rejected.', 422)->withHeader('Cache-Control', 'no-store');
        }
    }

    private function routeId(Request $request, string $key): ?EntityId
    {
