...
{
    public function testUnknownProviderIdentityLinksOnlyByVerifiedExistingEmail(): void
    {
        ...
        $transaction = self::transaction($state, $now);

        $transactions = $this->createMock(OAuthTransactionStore::class);
        $transactions->expects(self::once())->method('consume')->willReturn($transaction);
        ...
        self::assertSame($user, $service->complete(
            'google', $state, 'code', 'https://forum.example.test/oauth/google/callback', null,
        ));
    }

    public function testUnverifiedEmailCannotImplicitlyLinkAccount(): void
    {
        $now = new DateTimeImmutable('2026-09-15T12:45:00+00:00');
        $transactions = $this->createMock(OAuthTransactionStore::class);
        ...
        $service = $this->service(
            new OAuthIdentity('google', 'subject-2', 'member@example.com', false, 'Member'),
            $transactions,
            $accounts,
            $users,
            $now,
    ...
    public function testConnectedIdentityCannotBeLinkedToDifferentUser(): void
    {
        $now = new DateTimeImmutable('2026-09-15T12:45:00+00:00');
        $owner = self::user('owner@example.com', UserStatus::Active, $now);
        $attacker = self::user('attacker@example.com', UserStatus::Active, $now);
        $state = str_repeat('l', 32);
        $transaction = new OAuthTransaction(
            Pkce::stateHash($state), 'google', str_repeat('V', 43),
            'https://forum.example.test/oauth/google/callback', $attacker->id(), $now, $now->modify('+10 minutes'),
        );
        $transactions = $this->createMock(OAuthTransactionStore::class);
        $transactions->method('consume')->willReturn($transaction);
        $accounts = $this->createMock(ConnectedAccountStore::class);
        $accounts->method('find')->willReturn(new ConnectedAccount(
            $owner->id(), 'google', 'subject-3', 'owner@example.com', 'Owner', $now, $now,
        ));
        $users = $this->createMock(UserRepository::class);

        $service = $this->service(
            new OAuthIdentity('google', 'subject-3', 'owner@example.com', true, 'Owner'),
            $transactions,
            $accounts,
            $users,
            $now,
        ...
        $this->expectException(OAuthException::class);
        $service->complete(
            'google', $state, 'code', 'https://forum.example.test/oauth/google/callback', $attacker->id(),
        );
    }
    ...
    public function testLastAuthenticationMethodCannotBeUnlinked(): void
    {
        ...
        $accounts->method('findForUser')->willReturn(new ConnectedAccount(
            $user->id(), 'google', 'subject-4', 'member@example.com', 'Member', $now, $now,
        ));
        ...
        $service = $this->service(
            new OAuthIdentity('google', 'subject-4', 'member@example.com', true),
            $transactions,
            $accounts,
            $users,
            $now,
    ...
    public function testNonActiveLocalAccountCannotAuthenticateThroughOAuth(): void
    {
        $now = new DateTimeImmutable('2026-09-15T12:45:00+00:00');
        ...
        $accounts = $this->createMock(ConnectedAccountStore::class);
        $accounts->method('find')->willReturn(new ConnectedAccount(
            $user->id(), 'google', 'subject-5', 'banned@example.com', 'Banned', $now, $now,
        ));
        $users = $this->createMock(UserRepository::class);
        $users->method('find')->willReturn($user);

        $service = $this->service(
            new OAuthIdentity('google', 'subject-5', 'banned@example.com', true),
            $transactions,
            $accounts,
            $users,
            $now,
    ...
    private function service(
        ...
        $provider = new IdentityOAuthProvider($identity);
        $registry = new OAuthProviderRegistry([$provider], [new OAuthProviderConfig(
            'google', true, 'client-id', 'oauth.google.client_secret', ['https://forum.example.test/oauth/google/callback'],
        )]);
        $secrets = $this->createMock(SecretStore::class);
        $secrets->method('get')->with('oauth.google.client_secret')->willReturn('client-secret');
...
final readonly class IdentityOAuthProvider implements OAuthProvider
{
