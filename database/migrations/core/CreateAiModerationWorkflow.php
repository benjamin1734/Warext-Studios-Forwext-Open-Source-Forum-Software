...
final readonly class CreateAiModerationWorkflow implements Migration
{
    public function id(): MigrationId
    {
        return MigrationId::fromString('20260918030000_ai_moderation_workflow');
    }
    ...
    public function up(MigrationContext $context): void
    {
        $context->execute(new CompiledQuery(
            'CREATE TABLE IF NOT EXISTS forwext_ai_moderation_decisions ('
            ...
            . 'risk_score DECIMAL(7,6) UNSIGNED NOT NULL,'
            . 'action VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,'
            . 'fallback_reason VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL,'
            . 'human_override TINYINT(1) UNSIGNED NOT NULL DEFAULT 0,'
            . 'target_type VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL,'
            . 'target_id CHAR(32) CHARACTER SET ascii COLLATE ascii_bin NULL,'
            . 'created_at_utc DATETIME(6) NOT NULL,'
            . 'PRIMARY KEY (decision_id),'
            . 'KEY idx_forwext_ai_decision_fingerprint (content_fingerprint,created_at_utc,decision_id),'
            . 'KEY idx_forwext_ai_decision_action (action,created_at_utc,decision_id),'
            . 'KEY idx_forwext_ai_decision_target (target_type,target_id,created_at_utc)'
    ...
    public function verify(MigrationContext $context): MigrationVerification
    {
        $tables = $context->fetchValue(new CompiledQuery(
            ...
            . "AND TABLE_NAME IN ('forwext_ai_moderation_decisions','forwext_ai_moderation_overrides')",
        ));
        $decisionIndexes = $context->fetchValue(new CompiledQuery(
            'SELECT COUNT(DISTINCT INDEX_NAME) FROM information_schema.STATISTICS '
            . "WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='forwext_ai_moderation_decisions' "
            . "AND INDEX_NAME IN ('idx_forwext_ai_decision_fingerprint','idx_forwext_ai_decision_action','idx_forwext_ai_decision_target')",
        ));
        $overrideIndexes = $context->fetchValue(new CompiledQuery(
            'SELECT COUNT(DISTINCT INDEX_NAME) FROM information_schema.STATISTICS '
        ...
        return (int) $tables === 2
            && (int) $decisionIndexes === 3
            && (int) $overrideIndexes === 2
            && (int) $foreignKeys === 1
            && (int) $permission === 1
            && (int) $rules === 5
            ? MigrationVerification::passed()
            : MigrationVerification::failed('AI moderation workflow schema or permission defaults are incomplete.');
    }
}
