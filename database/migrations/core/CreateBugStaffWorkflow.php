...
final readonly class CreateBugStaffWorkflow implements Migration
{
    public function id(): MigrationId
    {
        return MigrationId::fromString('20260918025000_bug_staff_workflow');
    }
    ...
    public function up(MigrationContext $context): void
    {
        $context->execute(new CompiledQuery(
            ...
            . 'KEY idx_forwext_bug_duplicate_actor (created_by_user_id,created_at_utc),'
            . 'CONSTRAINT fk_forwext_bug_duplicate_source FOREIGN KEY (duplicate_report_id) '
        ...
        $permissions = [
            'bug.report.export'=>'Export authorized bug-report data.',
            'bug.audit.view'=>'View centralized bug-report audit records.',
        ];
    ...
    public function verify(MigrationContext $context): MigrationVerification
    {
        $table = $context->fetchValue(new CompiledQuery(
            ...
            . "AND TABLE_NAME='forwext_bug_report_duplicates'",
        ));
        $foreignKeys = $context->fetchValue(new CompiledQuery(
            'SELECT COUNT(*) FROM information_schema.REFERENTIAL_CONSTRAINTS '
            . 'WHERE CONSTRAINT_SCHEMA=DATABASE() AND CONSTRAINT_NAME IN '
            . "('fk_forwext_bug_duplicate_source','fk_forwext_bug_duplicate_canonical','fk_forwext_bug_duplicate_actor')",
        ));
        $indexes = $context->fetchValue(new CompiledQuery(
            "SELECT COUNT(DISTINCT INDEX_NAME) FROM information_schema.STATISTICS "
            . "WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='forwext_bug_report_duplicates' "
            . "AND INDEX_NAME IN ('idx_forwext_bug_duplicate_canonical','idx_forwext_bug_duplicate_actor')",
        ));
        $permissions = $context->fetchValue(new CompiledQuery(
            "SELECT COUNT(*) FROM forwext_permissions WHERE permission_key IN "
            . "('bug.report.export','bug.audit.view') AND value_type='flag'",
        ));
        $rules = $context->fetchValue(new CompiledQuery(
            ...
            . "AND permission_key IN ('bug.report.export','bug.audit.view')",
        ));
        ...
        return (int) $table === 1
            ...
            : MigrationVerification::failed('Bug staff workflow schema or permissions are incomplete.');
    }
}
