...
{
    public function __construct(private RedisClient $redis, private string $prefix = 'forwext:cache:', private Clock $clock = new SystemClock())
    {
    }
    ...
    public function get(string $key): ?CacheEntry
    {
        ...
        if (!is_array($decoded) || !is_string($decoded['value_b64'] ?? null)) {
            throw new InfrastructureException('Redis cache payload has an invalid shape.');
        }
        $value = base64_decode($decoded['value_b64'], true);
        if ($value === false) {
            throw new InfrastructureException('Redis cache value encoding is invalid.');
        }
        $expires = null;
        ...
        $entry = new CacheEntry($value, $expires, $tags);
    ...
    private function getWithoutExpiryCleanup(string $key): ?CacheEntry
    {
        ...
        if (!is_array($decoded) || !is_string($decoded['value_b64'] ?? null) || !is_array($decoded['tags'] ?? null)) {
            return null;
        }
        $value = base64_decode($decoded['value_b64'], true);
        if ($value === false) {
        ...
        /** @var list<string> $tags */
        return new CacheEntry($value, null, $tags);
    }
