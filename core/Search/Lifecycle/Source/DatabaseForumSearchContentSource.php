...
{
    public function documentType(): string { return 'forum'; }
    ...
    public function document(string $documentId): ?SearchDocument
    {
        ...
        $row = $this->database->fetchOne(new CompiledQuery(
            ...
            . 'FROM `forwext_nodes` WHERE `node_id`=:node_id LIMIT 1', ['node_id' => $documentId],
        ));
        if ($row === null || (string) ($row['visibility'] ?? '') === ForumNodeVisibility::Disabled->value) return null;
        foreach (['title','description','visibility','updated_at_utc'] as $key) {
            if (!is_string($row[$key] ?? null)) throw new SearchException('Stored forum search source is malformed.');
        }
        $body = trim((string) $row['description'] . "\n" . (string) ($row['page_content'] ?? ''));
        return new SearchDocument(
            $this->documentType(), $documentId, (string) $row['title'], $body,
            [SearchIndexScope::forumNode(EntityId::fromString($documentId))],
            $this->date((string) $row['updated_at_utc']), null,
            [SearchAttribute::FORUM => [$documentId], SearchAttribute::STATE => [(string) $row['visibility']]],
        );
    }

    public function scan(?string $afterId, int $limit): SearchContentPage { return $this->scanIds('forwext_nodes', 'node_id', $afterId, $limit); }
}
