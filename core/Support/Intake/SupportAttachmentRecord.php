...
final readonly class SupportAttachmentRecord
{
    ...
    public function __construct(
        ...
        if ($this->sizeBytes < 1 || preg_match('/^[a-f0-9]{64}$/D', $this->sha256) !== 1) {
            throw new InvalidArgumentException('Support attachment integrity metadata is invalid.');
        }
        if (preg_match('/^[a-z0-9.+-]+\/[a-z0-9.+-]+$/D', $this->mediaType) !== 1
            || preg_match('/^[a-z0-9]{1,10}$/D', $this->extension) !== 1
            || $this->storagePath === ''
        ) {
            throw new InvalidArgumentException('Support attachment storage metadata is invalid.');
        }
