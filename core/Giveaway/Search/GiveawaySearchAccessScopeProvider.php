<?php

declare(strict_types=1);

namespace Forwext\Core\Giveaway\Search;

use Forwext\Core\Domain\Access\Permission\PermissionAuthorizer;
use Forwext\Core\Domain\Access\Permission\PermissionKey;
use Forwext\Core\Domain\Entity\EntityId;
use Forwext\Core\Search\Access\SearchAccessScopeProvider;

final readonly class GiveawaySearchAccessScopeProvider implements SearchAccessScopeProvider
{
    public const MEMBERS = 'giveaway.members';

    public function __construct(private PermissionAuthorizer $authorizer)
    {
    }

    public function scopes(EntityId $userId): array
    {
        return $this->authorizer->allows($userId, PermissionKey::fromString('giveaway.view'))
            ? [self::MEMBERS]
            : [];
    }
}
