...
final readonly class BugAttachmentDownloadService
{
    ...
    public function download(EntityId $reportId,EntityId $attachmentId): AttachmentDownload
    {
        ...
        if($record===null){
            throw new BugAttachmentOperationException('Bug report attachment was not found.');
        }

        $contents=$this->storage->read(StoragePath::fromString($record->storagePath),StorageVisibility::Private);
        if(strlen($contents)!==$record->sizeBytes||!hash_equals($record->sha256,hash('sha256',$contents))){
            throw new BugAttachmentOperationException('Bug report attachment integrity check failed.');
        }

        return new AttachmentDownload($contents,$record->mediaType,$record->filename->value(),false);
    }
}
