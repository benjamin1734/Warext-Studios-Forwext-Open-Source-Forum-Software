...
final readonly class BugAttachmentDownloadService
{
    ...
    public function download(EntityId $reportId, EntityId $attachmentId): AttachmentDownload
    {
        ...
        if ($record === null) {
            throw new BugReportOperationException('Bug attachment was not found.');
        }
        ...
        if (
            strlen($contents) !== $record->sizeBytes
            || !hash_equals($record->sha256, hash('sha256', $contents))
        ) {
            throw new BugReportOperationException('Bug attachment integrity check failed.');
        }
