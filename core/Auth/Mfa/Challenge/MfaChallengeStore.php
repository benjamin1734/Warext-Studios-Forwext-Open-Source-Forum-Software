...
final readonly class MfaChallengeStore
{
    ...
    public function issue(
        ...
        foreach ([$identityFingerprint, $ipFingerprint, $deviceFingerprint] as $fingerprint) {
            if (preg_match('/^[a-f0-9]{64}$/D', $fingerprint) !== 1) {
                throw new MfaException('MFA challenge fingerprint is invalid.');
            }
        }
        ...
        if ($purpose === MfaChallengePurpose::Login && $actionKey !== null) {
            throw new MfaException('Login MFA challenge cannot carry an action key.');
        }

        $raw = 'mfa_' . bin2hex(random_bytes(32));
        $expires = $this->clock->now()->add(new DateInterval('PT' . $this->ttlSeconds . 'S'));
        $this->database->execute(new CompiledQuery(
            'INSERT INTO `forwext_mfa_challenges` '
            . '(`challenge_hash`,`user_id`,`device_id`,`credential_version`,`purpose`,`action_key`,`remember_requested`,`identity_fingerprint`,`ip_fingerprint`,`device_fingerprint`,`expires_at_utc`,`created_at_utc`,`consumed_at_utc`) '
            . 'VALUES (:challenge_hash,:user_id,:device_id,:credential_version,:purpose,:action_key,:remember_requested,:identity_fingerprint,:ip_fingerprint,:device_fingerprint,:expires_at_utc,:created_at_utc,NULL)',
            [
                'challenge_hash' => hash('sha256', $raw), 'user_id' => $userId->value(), 'device_id' => $deviceId,
                'credential_version' => $credentialVersion, 'purpose' => $purpose->value, 'action_key' => $actionKey,
                'remember_requested' => $rememberRequested ? 1 : 0, 'identity_fingerprint' => $identityFingerprint,
                'ip_fingerprint' => $ipFingerprint, 'device_fingerprint' => $deviceFingerprint,
                'expires_at_utc' => self::date($expires), 'created_at_utc' => self::date($this->clock->now()),
            ],
        ));
        return $raw;
    }

    public function inspect(string $rawToken): ?MfaChallengeGrant
    {
        if (preg_match('/^mfa_[a-f0-9]{64}$/D', $rawToken) !== 1) {
            return null;
        }
        return $this->hydrate($this->database->fetchOne(new CompiledQuery(
            'SELECT `user_id`,`device_id`,`credential_version`,`purpose`,`action_key`,`remember_requested`,`identity_fingerprint`,`ip_fingerprint`,`device_fingerprint`,`expires_at_utc`,`consumed_at_utc` '
            . 'FROM `forwext_mfa_challenges` WHERE `challenge_hash`=:challenge_hash LIMIT 1',
            ['challenge_hash' => hash('sha256', $rawToken)],
        )));
    }
    ...
    public function consume(string $rawToken): ?MfaChallengeGrant
    {
        ...
        return $this->database->transaction(function () use ($hash): ?MfaChallengeGrant {
            $row = $this->database->fetchOne(new CompiledQuery(
                'SELECT `user_id`,`device_id`,`credential_version`,`purpose`,`action_key`,`remember_requested`,`identity_fingerprint`,`ip_fingerprint`,`device_fingerprint`,`expires_at_utc`,`consumed_at_utc` '
                . 'FROM `forwext_mfa_challenges` WHERE `challenge_hash`=:challenge_hash LIMIT 1 FOR UPDATE',
                ['challenge_hash' => $hash],
    ...
    {
        if ($row === null || ($row['consumed_at_utc'] ?? null) !== null) {
            return null;
        }
        foreach (['user_id','device_id','purpose','identity_fingerprint','ip_fingerprint','device_fingerprint','expires_at_utc'] as $key) {
        ...
        return new MfaChallengeGrant(
            EntityId::fromString($row['user_id']), $row['device_id'], (int) ($row['credential_version'] ?? 0), $purpose,
            is_string($row['action_key'] ?? null) ? $row['action_key'] : null, (bool) ($row['remember_requested'] ?? false),
            $row['identity_fingerprint'], $row['ip_fingerprint'], $row['device_fingerprint'], $expires,
        );
    }
