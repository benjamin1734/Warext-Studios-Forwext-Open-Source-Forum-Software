...
final readonly class DatabaseRegistrationRateLimiter implements RegistrationRateLimiter
{
    ...
    public function consume(
        ...
        if (preg_match('/^[a-z][a-z0-9._-]{1,31}$/D', $scope) !== 1
            ...
            throw new RegistrationException('Registration rate-limit input is invalid.');
        }
        ...
        return $this->database->transaction(function () use ($scope, $fingerprint, $limit, $bucket): bool {
            $this->database->execute(new CompiledQuery(
                'INSERT INTO `forwext_registration_rate_limits` '
            ...
            $attempts = $this->database->fetchValue(new CompiledQuery(
                'SELECT `attempts` FROM `forwext_registration_rate_limits` '
